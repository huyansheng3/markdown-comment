...
> **(human)** 这张图需要补充各进程之间的通信协议说明，比如 IPC、RPC 等。
> **(ai)** 好的，我来补充通信协议的说明。
...
> **(human)** 这里可以补充一下为什么采用单进程多插件的设计，而不是每个插件一个进程？
> **(ai)** 主要是性能考虑：
> 1. 减少进程间通信开销
> 2. 降低内存占用
> 3. 简化插件间协作
...
> **(human)** 这个示例只展示了单向通信，能补充双向通信的完整示例吗？包括 Webview 端的接收和响应代码。
